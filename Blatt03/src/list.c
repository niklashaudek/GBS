...
struct list_elem *list_append (list_t *list, char *data) // fertig
{
    struct list_elem* pElement = (struct list_elem*) malloc(sizeof(struct list_elem));
    pElement->data = data;
    if (list->last != NULL)
...
struct list_elem *list_find (list_t *list,char *data, int (*cmp_elem) (const char *, const char*)) // fertig
{
    ...
    do
    {
        if (0 == (*cmp_elem) (comp->data, data)) { // Funktion zum Vergleichen der data anwenden
            return comp;
        }
        comp = comp->next;
    } while (comp != NULL);
