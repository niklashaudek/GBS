...
struct list_elem *list_append (list_t *list, int sd_new, struct sockaddr_in sin_new, struct timeval timeout_new) // fertig
{
    struct list_elem* pElement = (struct list_elem*) malloc(sizeof(struct list_elem));
    pElement->sd = sd_new;
    pElement->sin = sin_new;
    pElement->timeout = timeout_new;
