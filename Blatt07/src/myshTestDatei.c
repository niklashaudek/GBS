...
int main(int argc, char **argv, char *envp[])
{
    ...
    char *testarr[] = {"", "-l", "/bin/", NULL};
    int test = execve("/usr/sbin/ls", testarr , env); // Returns -1 on error
    exit(1);
        ...
        */
        ...
        for (int argCounter = 0; argCounter < cmdLineGeparst->size; argCounter++)
        {
            if ('<' == parseArray[argCounter][idx]) // lesen aus Datei
            {
                ...
                */
                close(0); // close STDIN
                int fd = open(parseArray[++argCounter], O_RDONLY, 0644); // Create file if not existing with rights 0644
                if(fd <= 0) {
...
/*
        char *testarr[] = {"", "-l", "/bin/"};
        int test = execve("/bin/ls", testarr , NULL); // Returns -1 on error
*/
        int result = 0;
